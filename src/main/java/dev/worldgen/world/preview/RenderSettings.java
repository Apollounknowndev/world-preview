...
import dev.worldgen.world.preview.backend.sampler.ChunkSampler;
import dev.worldgen.world.preview.backend.sampler.FullQuartSampler;
import dev.worldgen.world.preview.backend.sampler.QuarterQuartSampler;
import dev.worldgen.world.preview.backend.sampler.SingleQuartSampler;
...
import static dev.worldgen.world.preview.backend.WorkManager.Y_BLOCK_STRIDE;
import static dev.worldgen.world.preview.backend.storage.PreviewStorage.FLAG_BIOME;
import static dev.worldgen.world.preview.backend.storage.PreviewStorage.FLAG_HEIGHT;
import static dev.worldgen.world.preview.backend.storage.PreviewStorage.FLAG_INTERSECT;
import static dev.worldgen.world.preview.backend.storage.PreviewStorage.FLAG_NOISE_CONTINENTALNESS;
import static dev.worldgen.world.preview.backend.storage.PreviewStorage.FLAG_NOISE_DEPTH;
import static dev.worldgen.world.preview.backend.storage.PreviewStorage.FLAG_NOISE_EROSION;
import static dev.worldgen.world.preview.backend.storage.PreviewStorage.FLAG_NOISE_HUMIDITY;
import static dev.worldgen.world.preview.backend.storage.PreviewStorage.FLAG_NOISE_TEMPERATURE;
import static dev.worldgen.world.preview.backend.storage.PreviewStorage.FLAG_NOISE_WEIRDNESS;
...
public class RenderSettings {
    ...
    public void resetCenter() {
        center = new BlockPos(0, WorldPreview.workManager().yMax(), 0);
    }

    public void incrementY() {
        int nextY = (Math.min(center.getY() + Y_BLOCK_STRIDE, WorldPreview.workManager().yMax()) / Y_BLOCK_STRIDE) * Y_BLOCK_STRIDE;
        center = new BlockPos(center.getX(), nextY, center.getZ());
    }

    public void decrementY() {
        int nextY = (Math.max(center.getY() - Y_BLOCK_STRIDE, WorldPreview.workManager().yMin()) / Y_BLOCK_STRIDE) * Y_BLOCK_STRIDE;
        center = new BlockPos(center.getX(), nextY, center.getZ());
    }
