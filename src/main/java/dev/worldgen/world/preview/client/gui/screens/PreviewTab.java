...
import dev.worldgen.world.preview.backend.compat.LithostitchedCompat;
...
import dev.worldgen.world.preview.mixin.client.CreateWorldScreenAccessor;
...
import static dev.worldgen.world.preview.client.WorldPreviewComponents.LOADING_PREVIEW;
import static dev.worldgen.world.preview.client.WorldPreviewComponents.SAVING_PREVIEW;
...
public class PreviewTab implements Tab, AutoCloseable, PreviewContainerDataProvider {
    ...
    public @Nullable WorldCreationContext previewWorldCreationContext() {
        ...
        try {
            ...
            // Lithostitched only applies at server start, so do it here before the biome source is built.
            WorldDimensions.Complete worldDimensions = result.selectedDimensions().bake(result.datapackDimensions());
            LithostitchedCompat.apply(result.worldgenLoadContext(), worldDimensions.dimensions(), result.options().seed());
            return result;
    ...
    public Path cacheDir() {
        final Path previewDir = WorldPreview.CONFIG_FOLDER.resolve("world-preview");
